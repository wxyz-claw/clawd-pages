...
- **Embedding search** catches paraphrase. The person asking "restore hangs after manifest load" and the person who answered "checkpoint stalls on the NFS mount" may never share vocabulary. Vector similarity is what connects a question to an answer written in different words.
...
A worked example of how the four signals combine on one query:

```
Query: "restore hangs after manifest load"

1. [2w ago]  Thread  "Checkpoint stalls on the NFS mount — set CKPT_PREFETCH=4"
             -> EMBEDDING match (paraphrase, no shared tokens) + rare token + recent
2. [1d ago]  Message "ERR_MANIFEST_TIMEOUT: restore hangs after manifest load"
             -> FULL-TEXT exact tokens + rare error string
3. [3h ago]  Message "sounds good, thanks! will try that"
             -> FALSE NEIGHBOR: embedding-close but no rare tokens -> IDF sinks it
4. [8mo ago] Thread  "restore hangs after manifest load -> use LEGACY_FETCHER=1"
             -> matches, but AGE DECAY: 8 months old, stale infra -> penalized
```
...
Raw Slack text is keyword-searchable as soon as it lands because we maintain a Postgres full-text (GIN) index over the raw content. To enable useful vector search, however, we do some additional processing.
...
Each thread distills into one structured artifact, which is what actually gets embedded:
...
We embed these data points and write them into the shared embeddings table. The original transcript is not embedded directly. In our experiments, accuracy increased significantly when the thread was normalized into a consistent format. The additional metadata also gives the semantic match more useful signal.
...
To boost the signal from individual messages, we use bursting. A burst is a run of consecutive messages from the same author. We embed individual bursts with the thread topic prepended as context because sometimes the answer lives in one tangent message whose vocabulary never makes it into the thread summary. Burst embeddings make that message findable on its own.
...
Concretely, one file cascades from class -> methods -> inner blocks -> private fields:

```cpp
// checkpoint_loader.cc
class CheckpointLoader {                 // 1. try: CLASS
 public:
  Status LoadManifest(Path p) {          // 2. method: LoadManifest()
    auto bytes = fs_.Read(p);
    if (!bytes.ok()) return bytes.status();   // 3. if still large: IF
    manifest_ = ParseManifest(*bytes);
    return ValidateShards(manifest_);
  }
  Status WarmShardCache() {              // 2. method: WarmShardCache()
    for (const auto& shard : manifest_.shards()) {  // 3. if still large: FOR
      cache_.Pin(shard.key());
      stats_.Record(shard.bytes());
    }
    return OkStatus();
  }
  Status Restore(RestoreOptions opts) {  // 2. method: Restore()
 ...
 private:                                // 2. section: private fields
  FileSystem fs_;
  Manifest manifest_;
  ShardCache cache_;
};
...
```
SCORE(d) = sum over lists L of  1 / (60 + rank_L(d))     k = 60

Merged ranking (six retriever lists fused):
  0.0792   class: CheckpointLoader       (ckpt_loader.cc)
  0.0776   INC-82: slow restore
  0.0640   wiki: checkpoint format
  0.0640   thread: NFS stall
  0.0476   thread: cache warmup
  0.0476   runbook: NFS mounts
```
...
We send the original query and those candidates to a small reranker model. It gives each document a score from zero to ten, and we keep the top ten.
...
In the MCP integration, we expose retrieval building blocks as direct tools instead of hiding them behind one "answer this question" endpoint. These tools are intentionally simple and as LLM-free as possible so clients can query them quickly and cheaply.
